...
Convert this into a reuseable function by replacing `flow()` by `function()` and starting the pipeline with `enflow()`:
...
Then, you use it just as a plain function. The arguùents ending with `_` are also a good way to immediately spot those who are treated specially by the tidyeval mechanism! Here, we redo the analysis:
...
## Flow objects to test alternate scenarii
...
The biggest difference is that branches in git terms do not dynamically heritate objects from parents but **proto**/**Flow** objects do (in this case, **master** branch is indeed the ancestor). So, we could create a function `branch()` that does something like this:
