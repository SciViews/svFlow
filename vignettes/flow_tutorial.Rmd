...
The biggest difference is that branches in git terms do not dynamically inherit objects from parents but **proto**/**Flow** objects do (in this case, **master** branch is indeed the ancestor). So, we could create a function `branch()` that does something like this:
