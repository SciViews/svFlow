...
With the `Flow` object, you can continue the pipeline where you left it, because all the variables are recorded inside it.
...
With the `flow()` function, you can add variables to your `Flow` object, including those you want to keep as `quosures` (by ending their names with `_`):
...
The quosure is recorded both as `var` and `var_` for further reference. You could use `var` is the pipeline expression to manipulate the quosure directly, but you would most probably use `var_` which will unquote it transparently in NSE expressions. Here is the same pipeline as before, but with all the possible variables stored either as quosure, or as usual R objects inside the `Flow`:
...
Notice that, even standard variables, like `thresh1` or `thresh2` must be called `thresh1_` and `thresh2_` to look for them **inside** the `Flow` object. Otherwise, they will be looked for in the calling environment as usual. Also, the `Flow` object can be accessed and manipulated directly through `..`. Here, we show how to assigna variable `temp_data` inside the `Flow` object in the middle of the pipeline.
