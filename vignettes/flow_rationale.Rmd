...
1. It does not alter the expression evaluated, and the dot can be placed everywhere in the expression. It means that _any_ expression is suitable and is "pipe-aware" with this operator.
2. It is _explicit_, that is, you don't have to guess what will happen, the location of the dot replacement(s) in the expression is explictly indicated. It should makes it also easier to understand from a beginneR's point of view, providing he is not used to **magrittr** style, of course.
...
Note that a **quosure** is recorded as `var`, not `var_`! Indeed, everything works as if the trailing underscore was an unary sufixed operator applied to `var`, which converts it into a **quosure**.
